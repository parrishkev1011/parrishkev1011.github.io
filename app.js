/**
 * CapCut Template Dashboard - Complete Application
 */
...
let filters = { category: 'all', format: 'all', status: 'all', search: '' };
...
function generateId(category) {
    ...
    const existing = templates.filter(t => t.id.startsWith(prefix)).map(t => parseInt(t.id.split('-')[1])).filter(n => !isNaN(n));
...
function saveTemplate(templateData) {
    if (templateData.id) {
        ...
        if (index !== -1) templates[index] = enrichTemplate(templateData);
    } else {
...
function duplicateTemplate(id) {
    ...
    if (!original) return;
    const duplicate = { ...original, id: null, publishDate: new Date().toISOString().split('T')[0], exportsTotal: 0, title: `${original.title} (Copy)` };
...
function applyAllSuggestions() {
    if (confirm('Apply suggestions to all templates?')) {
        templates = templates.map(t => ({ ...t, status: t.statusSuggestion })).map(enrichTemplate);
        saveData();
        render();
    }
}
...
// Import/Export
function exportCSV() {
    if (templates.length === 0) return alert('No templates to export');
    const headers = ['ID', 'Title', 'Category', 'Format Family', 'Publish Date', 'Total Exports', 'Days Live', 'Exports/Day', 'Status', 'Hashtags', 'Notes'];
    const rows = templates.map(t => [t.id, `"${t.title.replace(/"/g, '""')}"`, t.category, `"${t.formatFamily.replace(/"/g, '""')}"`, t.publishDate, t.exportsTotal, t.daysLive, t.exportsPerDay, t.status, `"${(t.hashtags || '').replace(/"/g, '""')}"`, `"${(t.notes || '').replace(/"/g, '""')}"`]);
...
function exportJSON() {
    if (templates.length === 0) return alert('No templates to export');
    const data = { version: '1.0', exportDate: new Date().toISOString(), templates: templates.map(t => ({ id: t.id, title: t.title, category: t.category, formatFamily: t.formatFamily, publishDate: t.publishDate, exportsTotal: t.exportsTotal, status: t.status, hashtags: t.hashtags || '', notes: t.notes || '' })) };
    downloadFile(JSON.stringify(data, null, 2), 'capcut-templates-backup.json', 'application/json');
}
...
function importJSON(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!data.templates || !Array.isArray(data.templates)) throw new Error('Invalid file format');
            const merge = confirm('Merge with existing templates? OK=merge, Cancel=replace');
            if (!merge) templates = [];
            data.templates.forEach(t => {
                if (!t.title || !t.category || !t.formatFamily || !t.publishDate || t.exportsTotal === undefined) return;
                if (templates.find(existing => existing.id === t.id)) t.id = null;
                saveTemplate(t);
            });
        } catch (error) {
...
function getFilteredTemplates() {
    return templates.filter(t => {
        ...
        if (filters.search) {
            ...
            if (!t.title.toLowerCase().includes(search) && !(t.notes || '').toLowerCase().includes(search)) return false;
        }
        return true;
    });
}
...
function generateWeeklyReview() {
    const eligibleForScale = templates.filter(t => (t.status === 'Testing' || t.status === 'Hold') && t.statusSuggestion === 'Scale');
    const candidatesToKill = templates.filter(t => t.status !== 'Kill' && t.statusSuggestion === 'Kill');
    const top5 = [...templates].sort((a, b) => b.exportsPerDay - a.exportsPerDay).slice(0, 5);
    const formatStats = {};
    templates.forEach(t => {
        if (!formatStats[t.formatFamily]) formatStats[t.formatFamily] = { count: 0, totalExports: 0 };
    ...
    const formatStatsArray = Object.entries(formatStats).map(([family, stats]) => ({
        ...
        avgExportsPerDay: (templates.filter(t => t.formatFamily === family).reduce((sum, t) => sum + t.exportsPerDay, 0) / stats.count).toFixed(2)
...
function renderWeeklyReview() {
    ...
    scaleEl.innerHTML = review.eligibleForScale.length === 0 ? '<div class="review-empty">No templates ready to scale</div>' : review.eligibleForScale.map(t => `<div class="review-item"><strong>${t.id} - ${t.title}</strong><small>${t.exportsPerDay} exports/day (${t.daysLive} days live)</small></div>`).join('');
    const killEl = document.getElementById('reviewKill');
    killEl.innerHTML = review.candidatesToKill.length === 0 ? '<div class="review-empty">No templates to kill</div>' : review.candidatesToKill.map(t => `<div class="review-item"><strong>${t.id} - ${t.title}</strong><small>${t.exportsPerDay} exports/day (${t.daysLive} days live)</small></div>`).join('');
    const top5El = document.getElementById('reviewTop5');
    top5El.innerHTML = review.top5.length === 0 ? '<div class="review-empty">No templates yet</div>' : review.top5.map((t, i) => `<div class="review-item"><strong>#${i + 1} - ${t.id} - ${t.title}</strong><small>${t.exportsPerDay} exports/day (${t.exportsTotal} total)</small></div>`).join('');
    const formatsEl = document.getElementById('reviewFormats');
    formatsEl.innerHTML = review.formatStats.length === 0 ? '<div class="review-empty">No format families yet</div>' : review.formatStats.map(f => `<div class="review-item"><strong>${f.family}</strong><small>${f.count} templates • ${f.totalExports} exports • ${f.avgExportsPerDay} avg/day</small></div>`).join('');
}
...
function renderFormatFilter() {
    ...
    formatSelect.innerHTML = '<option value="all">All Formats</option>' + formats.map(f => `<option value="${f}">${f}</option>`).join('');
...
function renderTable() {
    ...
    tbody.innerHTML = sorted.map(t => {
        const suggestionText = t.status !== t.statusSuggestion ? `<br><small style="color: var(--color-warning);">💡 Suggests: ${t.statusSuggestion}</small>` : '';
        return `<tr><td><strong>${t.id}</strong></td><td>${escapeHtml(t.title)}</td><td>${t.category}</td><td>${escapeHtml(t.formatFamily)}</td><td>${formatDate(t.publishDate)}</td><td>${t.exportsTotal.toLocaleString()}</td><td>${t.daysLive}</td><td><strong>${t.exportsPerDay}</strong></td><td><span class="status-badge status-${t.status}">${t.status}</span>${suggestionText}</td><td><div class="template-actions"><button class="btn btn-small btn-secondary" onclick="editTemplate('${t.id}')">✏️ Edit</button><button class="btn btn-small btn-secondary" onclick="duplicateTemplate('${t.id}')">📋 Dup</button><button class="btn btn-small btn-danger" onclick="deleteTemplate('${t.id}')">🗑️ Del</button></div></td></tr>`;
    }).join('');
...
function openTemplateModal(template = null) {
    ...
    document.getElementById('modalTitle').textContent = template ? 'Edit Template' : 'Add Template';
...
function initEventListeners() {
    ...
    document.getElementById('filterFormat').addEventListener('change', (e) => { filters.format = e.target.value; render(); });
    document.getElementById('filterStatus').addEventListener('change', (e) => { filters.status = e.target.value; render(); });
    document.getElementById('filterSearch').addEventListener('input', (e) => { filters.search = e.target.value; render(); });
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !document.getElementById('templateModal').hidden) closeTemplateModal();
    });
